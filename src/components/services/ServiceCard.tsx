    ...
    >
      ...
      <CardContent className="space-y-6 min-w-0 flex-1">
        ...
        <div className="bg-[var(--brand-primary)]/5 border border-[var(--brand-primary)]/10 rounded-lg p-4 md:p-5">
          ...
          <p className="text-sm md:text-base text-[var(--brand-fg)]/90 leading-relaxed break-words">
            {service.suitableFor}
          </p>
        </div>
        ...
        {/* 제외 사항 */}
        <div>
          <div className="text-sm font-medium text-[var(--brand-fg)]/60 mb-3 uppercase tracking-wide">
            제외 사항
          </div>
          <ul className="space-y-2.5">
            {service.excludes.map((item, index) => (
              <li key={index} className="flex items-start gap-3 min-w-0">
                <XCircle className="w-5 h-5 text-muted-foreground/30 mt-0.5 flex-shrink-0" />
                <span className="text-sm md:text-base text-[var(--brand-fg)]/50 leading-relaxed break-words flex-1">
                  {item}
                </span>
              </li>
            ))}
